Ext.define('Proejct4.view.mobileview.resource.ResourcePanelController', {
	...
	onResourcePanelAfterRender:function()
	{
	    ...
	    Ext.Ajax.request({
	                 ...
	                 success : function(response,currentObject){
	                	  ...
	                	  currentObject.controller.createTreePanel(currentObject.controller, responseJson.response.data.menus);
	...
	createTreePanel : function(current, rawData)
	{
				...
				for (var j = 0; j < this.formResourcePanel.items.length; j++) {
					...
					if(me.formResourcePanel.items.items[j].title == record.menuLabel)
						{
							var treePanel = new Ext.create('Proejct4.view.mobileview.resource.AppMenuTreePanel');
